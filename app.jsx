  ...
  const handleCsv = () => {
    ...
    analysis.categoryAverages.forEach(cat => {
      cat.questions.forEach(q => {
        rows.push(['Q' + q.qNum, cat.label, q.header, q.average !== null ? q.average.toFixed(2) : '']);
      });
    });
...
function renderBarChartPng(items, opts = {}) {
  ...
  items.forEach((it, i) => {
    ...
    if (v != null) {
      ...
      ctx.fillText(v.toFixed(2), padL + w + 6, yCenter + 4);
    }
  });
...
async function exportToExcel(analysis, fileName = 'survey-analysis.xlsx') {
  ...
  qCols.forEach((qc, i) => {
    ...
    avgCell.value = {
      formula: `IFERROR((COUNTIF(${range},"${LIKERT_HDR[0]}")*5+COUNTIF(${range},"${LIKERT_HDR[1]}")*4+COUNTIF(${range},"${LIKERT_HDR[2]}")*3+COUNTIF(${range},"${LIKERT_HDR[3]}")*2+COUNTIF(${range},"${LIKERT_HDR[4]}")*1)/(COUNTIF(${range},"${LIKERT_HDR[0]}")+COUNTIF(${range},"${LIKERT_HDR[1]}")+COUNTIF(${range},"${LIKERT_HDR[2]}")+COUNTIF(${range},"${LIKERT_HDR[3]}")+COUNTIF(${range},"${LIKERT_HDR[4]}")),0)`
    };
    avgCell.numFmt = '0.00';
    r.eachCell((cell, c) => {
      cell.border = thinBorder;
  ...
  addBody(['Overall average', analysis.overallAverage != null ? Number(analysis.overallAverage.toFixed(2)) : '—']);
  ...
  analysis.categoryAverages.forEach(cat => {
    cat.questions.forEach(q => {
      const v = q.average;
      const low = v != null && v < 4.0;
      const r = qSheet.addRow(['Q' + q.qNum, cat.label, q.header, v != null ? Number(v.toFixed(2)) : '']);
      r.eachCell((cell, c) => {
        cell.border = thinBorder;
...
function generateAnalysisText(cats, label) {
  ...
  lines.push(`The category-level mean across the displayed bars is ${mean.toFixed(2)} on the 1–5 Likert scale.`);
  lines.push(`The strongest category is "${top.label}" with an average of ${top.average.toFixed(2)} (${top.count || top.respondents || 0} respondents).`);
  lines.push(`The weakest category is "${bottom.label}" with an average of ${bottom.average.toFixed(2)} (${bottom.count || bottom.respondents || 0} respondents).`);
  if (above.length) {
  ...
  if (below.length) {
    const names = below.map(c => `"${c.label}" (${c.average.toFixed(2)})`).join(', ');
  ...
  lines.push(`The spread between the highest and lowest category is ${spread.toFixed(2)} points, ${spread < 0.5 ? 'indicating a consistent experience across dimensions' : spread < 1.0 ? 'showing moderate variation across dimensions' : 'indicating uneven performance across dimensions'}.`);
...
async function exportPPTFromTemplate(analysis, fileName) {
  ...
  const overall = analysis.overallAverage != null ? analysis.overallAverage.toFixed(2) : '—';
  // Response rate: respondents / questionCount is what the template encodes as "11% (7 out of 62)"
  const responseRate = qCount > 0 ? Math.round((total / qCount) * 100) : 0;
  ...
  const analysisParts = [];
  analysisParts.push(
    `The ${courseLabel} course received an overall evaluation of ${overall} on the 1\u20135 Likert scale, ` +
    (parseFloat(overall) >= benchmark
      ? `which surpasses the ${benchmark.toFixed(1)} institutional benchmark and indicates strong student satisfaction.`
      : parseFloat(overall) >= 4.0
        ? `which exceeds the 4.0 satisfaction threshold but falls short of the ${benchmark.toFixed(1)} benchmark, leaving room for targeted improvement.`
        : `which falls below the 4.0 satisfaction threshold and signals systemic issues that warrant immediate attention.`)
  );
  if (strongest && weakest && strongest !== weakest) {
    analysisParts.push(
      `The strongest dimension is "${strongest.label.trim()}" (${strongest.average.toFixed(2)}), ` +
      `while the weakest is "${weakest.label.trim()}" (${weakest.average.toFixed(2)}), ` +
      `a spread of ${spread.toFixed(2)} points ` +
      (spread < 0.3 ? `that reflects a consistent experience across the survey dimensions.`
        : spread < 0.6 ? `that reveals moderate variation between dimensions and points to specific areas for refinement.`
        : `that highlights uneven performance and warrants a closer look at the lower-scoring areas.`)
    );
  }
  if (belowTarget.length > 0) {
    const names = belowTarget.map(c => `"${c.label.trim()}" (${c.average.toFixed(2)})`).join(', ');
    analysisParts.push(`Categories below the 4.0 satisfaction threshold are ${names} \u2014 these require corrective action and item-level review with the teaching team.`);
  } else if (belowMean.length > 0 && weakest) {
    analysisParts.push(`Although every category clears the 4.0 threshold, "${weakest.label.trim()}" sits below the course mean of ${meanAvg.toFixed(2)} and is the most actionable area for improvement in the next iteration.`);
  }
  if (total < 10) {
    analysisParts.push(`Note: with only ${total} respondent${total === 1 ? '' : 's'} out of ${qCount} enrolled, the sample is small and findings should be interpreted as directional rather than conclusive.`);
  } else if (responseRate < 30) {
    analysisParts.push(`The ${responseRate}% response rate is low; broader participation in future surveys would strengthen the reliability of these conclusions.`);
  }

  // Two concrete improvement suggestions tied to the data
  const suggestions = [];
  if (weakest) {
    const w = weakest.label.trim().toLowerCase();
    if (w.includes('beginning') || w.includes('start') || w.includes('outline')) {
      suggestions.push(`Suggested improvement: strengthen the opening weeks of "${weakest.label.trim()}" by publishing a clearer course outline, expected outcomes, and assessment plan in the first session, and adding a short orientation activity to ease students into the material.`);
    } else if (w.includes('learning outcome') || w.includes('outcomes')) {
      suggestions.push(`Suggested improvement: tighten the alignment between weekly activities and the stated learning outcomes in "${weakest.label.trim()}" \u2014 map each outcome to specific assessments and add formative checks so students can track their own progress.`);
    } else if (w.includes('e-learning') || w.includes('distance') || w.includes('online')) {
      suggestions.push(`Suggested improvement: invest in the e-learning experience for "${weakest.label.trim()}" by enriching the LMS with recorded micro-lectures, scheduled office hours, and structured discussion prompts to compensate for reduced in-person contact.`);
    } else if (w.includes('instructor') || w.includes('faculty') || w.includes('teaching')) {
      suggestions.push(`Suggested improvement: support the teaching team responsible for "${weakest.label.trim()}" with mid-term peer observation and a focused review of pacing, clarity, and student feedback, then re-survey at end of term.`);
    } else if (w.includes('during') || w.includes('happened')) {
      suggestions.push(`Suggested improvement: rebalance the in-class experience for "${weakest.label.trim()}" \u2014 add more hands-on practice, shorten lecture blocks, and introduce weekly check-ins so issues surface before the end of the term.`);
    } else {
      suggestions.push(`Suggested improvement: prioritise "${weakest.label.trim()}" in the next course iteration with a targeted action plan \u2014 review the lowest-scoring items, redesign the weakest activities, and gather mid-term feedback to confirm the changes are landing.`);
    }
  }
  if (belowTarget.length > 1) {
    suggestions.push(`Suggested improvement: because more than one dimension sits below the 4.0 threshold, convene a short retrospective with the teaching team, agree on two high-impact changes (e.g. revised assessment rubric, additional practice sessions), and schedule a follow-up pulse survey at mid-term.`);
  } else if (parseFloat(overall) >= benchmark && strongest) {
    suggestions.push(`Suggested improvement: capture and document what is working in "${strongest.label.trim()}" (lecture style, materials, pacing) so the same practices can be transferred to other courses in the programme as a benchmark exemplar.`);
  } else {
    suggestions.push(`Suggested improvement: introduce a brief mid-term student-feedback round (3\u20135 short questions) to detect issues early in the next offering, rather than relying solely on the end-of-course survey.`);
  }
  analysisParts.push(suggestions.join(' '));
  ...
  if (rawComments.length === 0) {
    ...
    try {
      const prompt =
        `You are summarizing open-ended student comments from a course evaluation. ` +
        `The comments may be written in Arabic, English, or a mix. ` +
        `Write EXACTLY two sentences in clear, natural English that capture the overall impression of the participants \u2014 ` +
        `include both positives and negatives or suggestions if present in the comments. ` +
        `Do NOT quote individual comments. Do NOT include any Arabic text in your response. ` +
        `Do not preface with "The students said" or similar; write a direct synthesis. ` +
        `Return ONLY the two sentences, nothing else.\n\n` +
        `Comments:\n${rawComments.map((c, i) => `${i + 1}. ${c}`).join('\n')}`;
      const reply = await window.claude.complete(prompt);
      const cleaned = String(reply || '').trim()
        .replace(/^["'\u201C\u201D]+|["'\u201C\u201D]+$/g, '')
        .replace(/\s+/g, ' ')
        .trim();
      // Strip any Arabic that slipped through
      const englishOnly = cleaned.replace(/[\u0600-\u06FF\u0750-\u077F]+/g, '').replace(/\s{2,}/g, ' ').trim();
      if (englishOnly && englishOnly.length > 20) {
        commentPara = englishOnly;
      } else {
        commentPara = `Participants submitted ${rawComments.length} open-ended comment${rawComments.length === 1 ? '' : 's'}; an automatic English summary could not be generated, so a manual review of the original responses is recommended.`;
      }
    } catch (e) {
      console.warn('Comment synthesis failed:', e);
      commentPara = `Participants submitted ${rawComments.length} open-ended comment${rawComments.length === 1 ? '' : 's'}; please review them directly in the source spreadsheet for qualitative insight.`;
    }
  } else {
    commentPara = `Participants submitted ${rawComments.length} open-ended comment${rawComments.length === 1 ? '' : 's'}; please review them directly in the source spreadsheet for qualitative insight.`;
  }
  ...
  slide2 = _replaceText(slide2, 'EMAI610 - Programing for AI', courseLabel);
  // Two "62" runs in the template: number-of-questions stat + the "out of 62" reference
  slide2 = slide2.replace(/<a:t>62<\/a:t>/g, `<a:t>${qCount}</a:t>`);
  slide2 = _replaceText(slide2, '11%', `${responseRate}%`);
  slide2 = _replaceText(slide2, '(7 out of 62)', `(${total} out of ${qCount})`);
  slide2 = _replaceText(slide2, '4.56', overall);
  // Replace the long analysis paragraph
  slide2 = slide2.replace(
    /<a:t>The Programming for AI[\s\S]*?high level of student satisfaction\.<\/a:t>/,
    `<a:t>${_xmlEscape(analysisPara)}</a:t>`
  );
  // Replace the older sample analysis paragraph too (the chart shows...)
  slide2 = slide2.replace(
    /<a:t>The chart shows[\s\S]*?above 4\.0\.<\/a:t>/,
    `<a:t>${_xmlEscape(analysisPara)}</a:t>`
  );
  // Replace the participant-comments paragraph
  slide2 = slide2.replace(
    /<a:t>Participants praised the curriculum[\s\S]*?<\/a:t>/,
...
async function exportPPT(analysis, fileName) {
  // Try template-based export first
  try {
    return await exportPPTFromTemplate(analysis, fileName);
  } catch (e) {
  ...
  function addChartSlide(title, cats, segmentLabel) {
    ...
    const chartData = [{
      ...
      values: cats.map(c => c.average !== null ? Number(c.average.toFixed(2)) : 0),
    }];
  ...
  // Slide 2: By gender (one chart with grouped categories per gender)
  {
    ...
    const series = analysis.byGender.map(g => ({
      ...
      labels,
      values: g.categories.map(c => c.average !== null ? Number(c.average.toFixed(2)) : 0),
    }));
  ...
  // Slide 3: By semester
  {
    ...
    const series = analysis.bySemester.map(s => ({
      ...
      labels,
      values: s.categories.map(c => c.average !== null ? Number(c.average.toFixed(2)) : 0),
    }));
    slide.addChart(pptx.ChartType.bar, series, {
      x: 0.5, y: 1.3, w: 7.5, h: 5.5,
      barDir: 'bar', barGrouping: 'clustered',
  ...
  return (
    <div className="app">
        ...
        {analysis && (
          <div className="header-actions">
            ...
            <button className="btn ghost" onClick={async () => {
              try { await exportPPT(analysis, fileName); }
      ...
      <main className="app-main">
        ...
        {analysis && <Dashboard analysis={analysis} view={view} setView={setView} />}
      </main>
        ...
        {items.map((it, i) => {
          ...
          return (
            <div key={i} className="mock-row">
              ...
              <div className="mock-val">{it.value.toFixed(2)}</div>
            </div>
          );
        })}
      </div>
...
function Dashboard({ analysis, view, setView }) {
  ...
  return (
    <div className="dash">
      <div className="kpi-row">
        <StatCard label="Total responses" value={analysis.totalResponses} />
          ...
          <div>
            ...
            {lowCats.map((c, i) => (
              <span key={c.id} className="alert-tag">
                {c.label} ({c.average.toFixed(2)}){i < lowCats.length - 1 ? ' · ' : ''}
              </span>
            ))}
          </div>
        </div>
                ...
                <div key={g.gender} className="chart-card">
                  <BarChart
                    ...
                    onDownloadCsv={() => {
                      ...
                      analysis.categoryAverages.forEach((cat, i) => {
                        const v = g.categories[i].average;
                        rows.push([cat.label, v !== null ? v.toFixed(2) : '']);
                      });
