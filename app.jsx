...
async function exportPPTFromTemplate(analysis, fileName) {
  ...
  // Build participant-comments paragraph from real student free-text answers
  let commentPara;
  const rawComments = (analysis.comments || [])
    .map(c => c.text.replace(/\s+/g, ' ').trim())
    .filter(t => t.length >= 4 && t.length <= 400)
    .filter(t => !/^(no|none|n\/a|na|nothing|nil|\-+|\.+)\.?$/i.test(t));
  if (rawComments.length === 0) {
    commentPara = `No open-ended comments were submitted by participants in this survey.`;
  } else {
    // Pick up to 3 representative comments and translate any Arabic to English via Claude
    const picked = rawComments.slice(0, 3);
    const hasArabic = (s) => /[\u0600-\u06FF]/.test(s);
    let translated = picked;
    if (picked.some(hasArabic) && window.claude && typeof window.claude.complete === 'function') {
      try {
        const prompt =
          `Translate any Arabic text in the following student survey comments to clear, natural English. ` +
          `If a comment is already in English, keep it as-is. Return ONLY a JSON array of strings, one per input, in the same order. ` +
          `Do not add quotation marks around the strings beyond JSON syntax. Do not include the original Arabic.\n\n` +
          `Comments:\n${JSON.stringify(picked)}`;
        const reply = await window.claude.complete(prompt);
        const match = reply.match(/\[[\s\S]*\]/);
        if (match) {
          const arr = JSON.parse(match[0]);
          if (Array.isArray(arr) && arr.length === picked.length) {
            translated = arr.map(s => String(s).trim());
          }
        }
      } catch (e) {
        console.warn('Translation failed, using originals:', e);
      }
    }
    // Drop any that are still Arabic-only after translation attempt
    const finalComments = translated.filter(t => !hasArabic(t) || t.length > 0).slice(0, 2);
    const quoted = finalComments.map(t => {
      const clean = t.replace(/[\u201C\u201D"]/g, '').trim();
      return `\u201C${clean}\u201D`;
    });
    commentPara = `Selected participant comments (translated from Arabic where applicable): ${quoted.join(' \u2014 ')}.`;
  }
