...
function analyze({ headers, rows }) {
  ...
  return {
    headers, rows, cols, meta, questionCols, records,
...
async function exportPPTFromTemplate(analysis, fileName) {
  ...
  // Build analysis paragraph (1-2 sentences from generateAnalysisText, joined)
  const analysisCats = cats.map(c => ({ label: c.label, average: c.average || 0, count: c.respondents || 0 }));
  const lines = generateAnalysisText(analysisCats, 'this course');
  const analysisPara = lines.slice(0, 4).join(' ');
  ...
  slide2 = slide2.replace(
    ...
    `<a:t>${_xmlEscape(lines.slice(4, 7).join(' ') || lines.join(' '))}</a:t>`
  );
